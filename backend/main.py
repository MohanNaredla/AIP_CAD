...
async def load_data_and_model():
    global df, model
    df = pd.read_excel("Final.xlsx", engine="openpyxl", index_col=0, nrows=1000)
...
async def get_students():
    ...
    for _, row in df.iterrows():
        ...
        students.append({"id": str(sid), "grade": grade_str})
...
async def get_student_details(student_id: int):
    ...
    return {
        "attendance2024": a24,
        "predicted2025": p25,
        "risk": {"level": level, "color": color, "description": desc},
        "predictedAttendance": {
            "year": "2025",
            "attendanceRate": p25,
            "absences": unexc + exc,
